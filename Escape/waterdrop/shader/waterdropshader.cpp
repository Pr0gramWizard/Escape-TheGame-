...
#include "Waterdropshader.hpp"


// Constructor
WaterdropShader::WaterdropShader(const std::string& pVertexShaderFilePath, const std::string& pFragementShaderFilePath)
{
	...
	// Log Shader
	std::clog << "Waterdrop Shader class was created successfully!" << std::endl;
}

// Compilation of the vertex and fragment shader 
// Function: complieShader(Filepath, Filepath)
void WaterdropShader::createShader(const std::string& pVertexShaderFilePath, const std::string& pFragementShaderFilePath)
{
	...
	{
		// Success message
		std::clog << "Waterdrop Vertex shader created successfully! " << std::endl;
	}
	...
	{
		// Success message
		std::clog << "Waterdrop Fragment shader created successfully! " << std::endl;
	}
...
void WaterdropShader::linkShader()
{
...
void WaterdropShader::compileFragementShader(std::string pFragementShaderFilePath)
{
...
void WaterdropShader::compileVertexShader(std::string pVertexShaderFilePath)
{
	// Opening a file stream with the given file path
...
GLuint WaterdropShader::getProgramID() const
{
	return mProgramID;
}
// Returns current Vertex Shader ID
GLuint WaterdropShader::getVertexShaderID() const
{
	return mVertexShaderID;
}
// Returns current Fragement Shader ID
GLuint WaterdropShader::getFragementShaderID() const
{
	return mFragementShaderID;
}
GLuint WaterdropShader::getUniformLocation(const char* pUniformName)
{
...
void WaterdropShader::setProgramID(int pProgramID)
{
	mProgramID = pProgramID;
}
// Sets current VertexShaderID to a given ID
void WaterdropShader::setVertexShaderID(int pVertexShaderID)
{
	mVertexShaderID = pVertexShaderID;
}
// Sets current FragementShaderID to a given ID
void WaterdropShader::setFragementShaderID(int pFragementShaderID)
{
	mFragementShaderID = pFragementShaderID;
}

void WaterdropShader::loadFloat(GLuint pLocation, GLfloat pValue)
{
	glUniform1f(pLocation, pValue);
}

void WaterdropShader::loadVector(GLuint pLocation, glm::vec3 pVector)
{
	glUniform3f(pLocation, pVector.x, pVector.y, pVector.z);
}

void WaterdropShader::loadBool(GLuint pLocation, GLboolean pValue)
{
	if (pValue == 0)
	{
...
void WaterdropShader::loadMatrix(GLuint pLocation, glm::mat4 pMatrix)
{
	glUniformMatrix4fv(pLocation, 1, GL_FALSE, glm::value_ptr(pMatrix));
}

void WaterdropShader::loadVector4f(GLuint pLocation, glm::vec4 pVector)
{
	glUniform4f(pLocation, pVector.x, pVector.y, pVector.z, pVector.w);
}

void WaterdropShader::loadModelMatrix(glm::mat4 pMatrix)
{
	loadMatrix(mLocation_modelMatrix, pMatrix);
}

void WaterdropShader::loadProjectionMatrix(glm::mat4 pMatrix)
{
	loadMatrix(mLocation_projectionMatrix, pMatrix);
}

void WaterdropShader::loadViewMatrix(glm::mat4 pMatrix)
{
	loadMatrix(mLocation_viewMatrix, pMatrix);
}

void WaterdropShader::loadLights(std::vector<Light*> pLights, bool pDiscoTime)
{
	if (pDiscoTime) {
...
void WaterdropShader::loadPlane(glm::vec4 pVector)
{
	this->loadVector4f(mLocation_plane, pVector);
}

void WaterdropShader::loadFogData(GLfloat pDensity, GLfloat pGradient)
{
	this->loadFloat(mLocation_fogDensity, pDensity);
	this->loadFloat(mLocation_fogGradient, pGradient);
}

void WaterdropShader::loadBackgroundColor(GLfloat pRed, GLfloat pGreen, GLfloat pBlue)
{
...
void WaterdropShader::getAllUniformLocations()
{
...
void WaterdropShader::bindAttribute(GLuint pAttribute, const std::string & pAttributeName)
{
...
void WaterdropShader::bindAllAttributes()
{
