...
GuiShader::GuiShader(const std::string& pVertexShaderFilePath, const std::string& pFragementShaderFilePath){
	this->createShader(pVertexShaderFilePath, pFragementShaderFilePath);
	this->getAllUniformLocations();
	this->bindAllAttributes();

	// Log Shader
	std::clog << "GUI Shader class was created successfully!" << std::endl;
}


// Compilation of the vertex and fragment shader 
// Function: complieShader(Filepath, Filepath)
void GuiShader::createShader(const std::string& pVertexShaderFilePath, const std::string& pFragementShaderFilePath)
{
	...
	{
		// Success message
		std::clog << "Gui Vertex shader created successfully! " << std::endl;
	}
	...
	{
		// Success message
		std::clog << "Gui Fragment shader created successfully! " << std::endl;
	}
...
void GuiShader::compileFragementShader(std::string pFragementShaderFilePath)
{
...
void GuiShader::compileVertexShader(std::string pVertexShaderFilePath)
{
	// Opening a file stream with the given file path
...
GLuint GuiShader::getProgramID() const
{
	return mProgramID;
}
// Returns current Vertex Shader ID
GLuint GuiShader::getVertexShaderID() const
{
	return mVertexShaderID;
}
// Returns current Fragement Shader ID
GLuint GuiShader::getFragementShaderID() const
{
	return mFragementShaderID;
}
GLuint GuiShader::getUniformLocation(const char* pUniformName)
{
...
void GuiShader::setProgramID(int pProgramID)
{
	mProgramID = pProgramID;
}
// Sets current VertexShaderID to a given ID
void GuiShader::setVertexShaderID(int pVertexShaderID)
{
	mVertexShaderID = pVertexShaderID;
}
// Sets current FragementShaderID to a given ID
void GuiShader::setFragementShaderID(int pFragementShaderID)
{
	mFragementShaderID = pFragementShaderID;
}

void GuiShader::loadFloat(GLuint pLocation, GLfloat pValue)
{
	glUniform1f(pLocation, pValue);
}

void GuiShader::loadVector(GLuint pLocation, glm::vec3 pVector)
{
	glUniform3f(pLocation, pVector.x, pVector.y, pVector.z);
}

void GuiShader::loadBool(GLuint pLocation, GLboolean pValue)
{
	if (pValue == 0)
	{
...
void GuiShader::loadMatrix(GLuint pLocation, glm::mat4 pMatrix)
{
	glUniformMatrix4fv(pLocation, 1, GL_FALSE, glm::value_ptr(pMatrix));
}

void GuiShader::loadVector4f(GLuint pLocation, glm::vec4 pVector)
{
	glUniform4f(pLocation, pVector.x, pVector.y, pVector.z, pVector.w);
}

void GuiShader::loadModelMatrix(glm::mat4 pMatrix)
{
	loadMatrix(mLocation_modelMatrix, pMatrix);
}

void GuiShader::loadProjectionMatrix(glm::mat4 pMatrix)
{
	loadMatrix(mLocation_projectionMatrix, pMatrix);
}

void GuiShader::loadViewMatrix(glm::mat4 pMatrix)
{
	loadMatrix(mLocation_viewMatrix, pMatrix);
}

void GuiShader::loadLight(Light *pLight)
{
...
void GuiShader::loadPlane(glm::vec4 pVector)
{
	this->loadVector4f(mLocation_plane, pVector);
}

void GuiShader::loadFogData(GLfloat pDensity, GLfloat pGradient)
{
	this->loadFloat(mLocation_fogDensity, pDensity);
	this->loadFloat(mLocation_fogGradient, pGradient);
}

void GuiShader::loadBackgroundColor(GLfloat pRed, GLfloat pGreen, GLfloat pBlue)
{
	this->loadVector(mLocation_backgroundColor, glm::vec3(pRed, pGreen, pBlue));
}

void GuiShader::loadTransformationMatrix(glm::mat4 pTransformationMatrix){
	loadMatrix(mLocation_transformationMatrix, pTransformationMatrix);
}
...
void GuiShader::getAllUniformLocations()
{
	...
	mLocation_transformationMatrix = glGetUniformLocation(getProgramID(), "transformation");
	// Light locations
...
void GuiShader::bindAttribute(GLuint pAttribute, const std::string & pAttributeName)
{
...
void GuiShader::bindAllAttributes()
{
