...
#include "Torchshader.hpp"


// Constructor
TorchShader::TorchShader(const std::string& pVertexShaderFilePath, const std::string& pFragementShaderFilePath)
{
	...
	// Log Shader
	std::clog << "Torch Shader class was created successfully!" << std::endl;
}

// Compilation of the vertex and fragment shader 
// Function: complieShader(Filepath, Filepath)
void TorchShader::createShader(const std::string& pVertexShaderFilePath, const std::string& pFragementShaderFilePath)
{
	...
	{
		// Success message
		std::clog << "Torch Vertex shader created successfully! " << std::endl;
	}
	...
	{
		// Success message
		std::clog << "Torch Fragment shader created successfully! " << std::endl;
	}
...
void TorchShader::compileFragementShader(std::string pFragementShaderFilePath)
{
...
void TorchShader::compileVertexShader(std::string pVertexShaderFilePath)
{
	// Opening a file stream with the given file path
...
GLuint TorchShader::getProgramID() const
{
	return mProgramID;
}
// Returns current Vertex Shader ID
GLuint TorchShader::getVertexShaderID() const
{
	return mVertexShaderID;
}
// Returns current Fragement Shader ID
GLuint TorchShader::getFragementShaderID() const
{
	return mFragementShaderID;
}
GLuint TorchShader::getUniformLocation(const char* pUniformName)
{
...
void TorchShader::setProgramID(int pProgramID)
{
	mProgramID = pProgramID;
}
// Sets current VertexShaderID to a given ID
void TorchShader::setVertexShaderID(int pVertexShaderID)
{
	mVertexShaderID = pVertexShaderID;
}
// Sets current FragementShaderID to a given ID
void TorchShader::setFragementShaderID(int pFragementShaderID)
{
	mFragementShaderID = pFragementShaderID;
}

void TorchShader::loadFloat(GLuint pLocation, GLfloat pValue)
{
	glUniform1f(pLocation, pValue);
}

void TorchShader::loadVector(GLuint pLocation, glm::vec3 pVector)
{
	glUniform3f(pLocation, pVector.x, pVector.y, pVector.z);
}

void TorchShader::loadBool(GLuint pLocation, GLboolean pValue)
{
	if (pValue == 0)
	{
...
void TorchShader::loadMatrix(GLuint pLocation, glm::mat4 pMatrix)
{
	glUniformMatrix4fv(pLocation, 1, GL_FALSE, glm::value_ptr(pMatrix));
}

void TorchShader::loadVector4f(GLuint pLocation, glm::vec4 pVector)
{
	glUniform4f(pLocation, pVector.x, pVector.y, pVector.z, pVector.w);
}

void TorchShader::loadModelMatrix(glm::mat4 pMatrix)
{
	loadMatrix(mLocation_modelMatrix, pMatrix);
}

void TorchShader::loadProjectionMatrix(glm::mat4 pMatrix)
{
	loadMatrix(mLocation_projectionMatrix, pMatrix);
}

void TorchShader::loadViewMatrix(glm::mat4 pMatrix)
{
	loadMatrix(mLocation_viewMatrix, pMatrix);
}

void TorchShader::loadLights(std::vector<Light*> pLights, bool pDiscoTime)
{
	if (pDiscoTime) {
...
void TorchShader::bindAttribute(GLuint pAttribute, const std::string & pAttributeName)
{
...
void TorchShader::bindAllAttributes()
{
